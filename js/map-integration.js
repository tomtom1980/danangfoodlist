...
(function() {
  ...
  function isRestaurantOpen(openingHours) {
    // Handle missing or invalid data - treat as CLOSED
    ...
    try {
      ...
      const currentDay = vietnamTime.getDay(); // 0=Sunday, 1=Monday, etc.
      ...
      // Check for day-specific closures (e.g., "CLOSED MONDAYS", "Sun closed")
      const dayNames = ['sun', 'mon', 'tue', 'wed', 'thu', 'fri', 'sat'];
      const closedDayMatch = normalized.match(/closed\s*(sun|mon|tue|wed|thu|fri|sat)/i) ||
                             normalized.match(/(sun|mon|tue|wed|thu|fri|sat)\s*closed/i);
      if (closedDayMatch) {
        const closedDay = dayNames.indexOf(closedDayMatch[1].toLowerCase().substring(0, 3));
      ...
      while ((match = timeRangeRegex.exec(normalized)) !== null) {
        ...
        // Handle variable closing times like "21:00/22:00" - use the later time
        const variableClose = closeTime.match(/(\d{1,2}(?::\d{2})?)\s*\/\s*(\d{1,2}(?::\d{2})?)/);
        if (variableClose) {
          // Use the later closing time
          const time1 = parseTime(variableClose[1]);
          const time2 = parseTime(variableClose[2]);
          closeTime = (time2 > time1) ? variableClose[2] : variableClose[1];
        }
      ...
      // Couldn't parse - treat as closed
      return false;
    } catch (error) {
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // Start with no categories selected (empty map on load)
  ...
  function setupMapEvents() {
    ...
    map.on('click', 'unclustered-point', function(e) {
      ...
      // Pass click coordinates for positioning
      showRestaurantPopup(feature, e.point);
    });
  ...
  function showRestaurantPopup(feature, clickPoint) {
    ...
    detailsBtn.onclick = function() {
      ...
      openRestaurantFromMap(props.category, props.name);
    };
    ...
    if (clickPoint && modal) {
      ...
      const clickX = mapRect.left + clickPoint.x;
      const clickY = mapRect.top + clickPoint.y;
      ...
      // Decide positioning: prefer to the right of click, then left, then center
      let left, top;

      // Check if modal fits to the right of click point
      if (clickX + offset + modalWidth < viewportWidth - 20) {
        left = clickX + offset;
      }
      // Check if modal fits to the left of click point
      ...
      // Vertical: prefer below click, then above, then center
      if (clickY + offset + modalHeight < viewportHeight - 20) {
        top = clickY + offset;
      } else if (clickY - offset - modalHeight > 20) {
        top = clickY - offset - modalHeight;
      } else {
        top = Math.max(20, Math.min(clickY - modalHeight / 2, viewportHeight - modalHeight - 20));
      }
  ...
  function updateFilterCount() {
    ...
    // Count restaurants matching all active filters (same logic as getFilteredData)
    const visibleCount = restaurantData.features.filter(f => {
      // Must match category
      if (!activeFilters.has(f.properties.categoryId)) return false;

      // Check open now filter
      if (showOpenNowOnly && !isRestaurantOpen(f.properties.openingHours)) return false;

      // Check distance filter
      if (!isWithinDistance(f)) return false;
    ...
    if (status) {
      ...
      status.textContent = `Showing ${visibleCount}${filterText} of ${restaurantData.totalCount} restaurants`;
    }
    if (toggleCount) {
