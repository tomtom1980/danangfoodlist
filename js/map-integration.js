...
(function () {
  ...
  // Deduplicated restaurant data cache (for accurate cluster counts)
  let deduplicatedRestaurantData = null;

  /**
   * Get deduplicated restaurant data (removes duplicates based on mapsLink)
   * This ensures cluster counts show unique physical locations, not category duplicates
   */
  function getDeduplicatedData() {
    if (deduplicatedRestaurantData) {
      return deduplicatedRestaurantData;
    }

    // Create a Set to track unique mapsLinks
    const seenMapsLinks = new Set();
    const uniqueFeatures = [];

    restaurantData.features.forEach(feature => {
      const mapsLink = feature.properties.mapsLink;
    ...
    deduplicatedRestaurantData = {
      type: 'FeatureCollection',
      features: uniqueFeatures
    };

    console.log(`Deduplicated restaurants: ${restaurantData.features.length} → ${uniqueFeatures.length} unique`);
    return deduplicatedRestaurantData;
  }
  ...
  function addMapSources() {
    ...
    // Use deduplicated data so cluster counts show unique physical locations
    map.addSource('restaurants', {
      type: 'geojson',
      data: getDeduplicatedData(),
      cluster: true,
      clusterMaxZoom: 14,
      clusterRadius: 50,
      clusterProperties: {
  ...
  function setupMapEvents() {
    ...
    map.on('click', 'unclustered-point', function (e) {
      ...
      // Show popup immediately without moving the map
      // Pass null for clickPoint so popup centers in the map container
      showRestaurantPopup(feature, null);
    });
  ...
  function showRestaurantPopup(feature, clickPoint) {
    ...
    // Position modal in the map container
    if (modal) {
      ...
      // Always center the modal in the map container
  ...
  function applyFilters() {
    ...
    // Expose filter state globally for slot machine
    window.mapActiveFilters = activeFilters;
    window.mapShowOpenNowOnly = showOpenNowOnly;
    window.mapShowTopPlacesOnly = showTopPlacesOnly;
    window.mapFilteredData = filteredData;

    // Dispatch custom event for slot machine
    document.dispatchEvent(new CustomEvent('filtersChanged', {
      detail: { count: filteredData.features.length }
    }));
  }

  /**
   * Get filtered GeoJSON data (deduplicated)
   * Top 3 filter is applied AFTER other filters so it respects distance/open-now
   */
  function getFilteredData() {
    const seenMapsLinks = new Set();
    let filteredFeatures = [];

    // Step 1: Apply category, open-now, and distance filters (but NOT top 3 yet)
    restaurantData.features.forEach(feature => {
      ...
      if (!isWithinDistance(feature)) return;

      // Deduplicate based on mapsLink (same physical location)
      const mapsLink = feature.properties.mapsLink;
      if (!seenMapsLinks.has(mapsLink)) {
        seenMapsLinks.add(mapsLink);
    ...
    // Step 2: If "Top 3 Places" filter is enabled, apply it now
    if (showTopPlacesOnly) {
      filteredFeatures = applyTop3Filter(filteredFeatures);
    }
  ...
  /**
   * Apply Top 3 filter: for each category, keep only the top 3 rated restaurants
   * @param {Array} features - Already filtered features
   * @returns {Array} Features with only top 3 per category
   */
  function applyTop3Filter(features) {
    ...
    features.forEach(feature => {
      ...
      byCategory.get(categoryName).push(feature);
    });

    // For each category, sort by rating (descending) and take top 3
    const result = [];
    
    byCategory.forEach((categoryFeatures, categoryName) => {
      ...
      categoryFeatures.sort((a, b) => {
        ...
        const reviewsA = parseInt(a.properties.reviewCount) || 0;
        const reviewsB = parseInt(b.properties.reviewCount) || 0;
        return reviewsB - reviewsA;
      });
      
      // Take top 3
      const top3 = categoryFeatures.slice(0, 3);
      result.push(...top3);
    });
   ...
   * Uses getFilteredData() to ensure count matches displayed results
   */
  function updateFilterCount() {
    const status = document.getElementById('filter-status');
    const toggleCount = document.getElementById('filter-toggle-count');

    // Get the actual filtered data (this already handles Top 3, distance, open-now)
    const filteredData = getFilteredData();
  ...
  window.openRestaurantFromMap = function (categoryName, restaurantName, mapsLink) {
    ...
    // First try: match by Google Maps link (most reliable - handles duplicate names)
    if (mapsLink) {
      for (const item of allRestaurantItems) {
        const linkEl = item.querySelector('.restaurant-item-maps');
        const nameEl = item.querySelector('.restaurant-item-name');
        if (linkEl && linkEl.href === mapsLink) {
          // If we have both link match AND name match, this is definitely the right one
          if (nameEl && normalizeString(nameEl.textContent) === normalizedSearchName) {
    ...
    // Second try: mapsLink only (in case name doesn't match exactly)
    ...
    // Third try: exact match by normalized name only
    if (!foundCardId) {
      for (const item of allRestaurantItems) {
        const nameEl = item.querySelector('.restaurant-item-name');
        if (nameEl) {
